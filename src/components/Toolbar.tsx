  ...
  return (
    <div className='table-selector'>
      {[...new Array(8)].map((_, row) => (
        <div
          key={`row-${row}`}
          className='table-selector-row'
        >
          {[...new Array(8)].map((_, col) => (
            <div
              key={`col-${col}`}
              className={`table-selector-cell ${
                row <= hoveredRow && col <= hoveredCol ? 'highlighted' : ''
              }`}
              onMouseEnter={() => handleSellHover(row, col)}
...
const Toolbar: FC<ToolbarProps> = ({
  ...
  const saveSelection = () => {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      try {
        ...
        console.warn(
          'Could not highlight complex selection (likely spans multiple elements)',
          e
        );
      }
    }
  ...
  const detectFormatting = useCallback(() => {
    ...
    const editorElement = document.getElementById('content-area');
    ...
    newActiveFormat.justifyLeft = normalizedAlign === 'left';
    newActiveFormat.justifyCenter = normalizedAlign === 'center';
    newActiveFormat.justifyRight = normalizedAlign === 'right';
    newActiveFormat.justifyFull = normalizedAlign === 'justify';
    ...
    setActiveFormats((prev) => {
      const hasChanged = Object.keys(newActiveFormat).some(
        (key) => prev[key] !== newActiveFormat[key]
      ) || Object.keys(prev).length !== Object.keys(newActiveFormat).length;

      return hasChanged ? newActiveFormat : prev;
    });
  }, []);
  ...
  return (
    <div
      id='toolbar'
      className={`toolbar ${
        toolbarClassName ? toolbarClassName : 'default-toolbar-class'
      }`}
    ...
    >
      ...
      >
        ...
        >
          <ClearFormatIcon className='button-icon' />
        </button>
      </div>
      ...
      >
        <div className='alignment-container'>
          <button
            type='button'
            className={isSelectAlign ? 'active' : ''}
          >
            {' '}
            {activeFormats?.justifyLeft ? (
              <AlignLeftIcon className='button-icon' />
            ) : activeFormats?.justifyCenter ? (
              <AlignCenterIcon className='button-icon' />
            ) : activeFormats?.justifyRight ? (
              <AlignRightIcon className='button-icon' />
            ) : activeFormats?.justifyFull ? (
              <AlignJustifyIcon className='button-icon' />
            ) : (
              <AlignLeftIcon className='button-icon' />
            )}
          </button>
            ...
            <div className='alignment-selector-show'>
              <div className='alignment-option-button'>
                ...
                <button
                  ...
                  className={activeFormats.justifyFull ? 'active' : ''}
                >
                  <AlignJustifyIcon className='button-icon' />
                </button>
              </div>
      ...
      >
        ...
        >
          <ListNumberIcon className='button-icon' />
        </button>
      </div>
      ...
      >
        <div className='table-container'>
          <button type='button'>
            <TableIcon className='button-icon' />
          </button>
      ...
      >
              ...
              <div className='link-insert-option'>
                {selectedText && (
                  <div className='link-selected-text'>
                    ...
                    <button
                      type='button'
                      className='link-unselect-button'
        ...
        >
          <LinkRemoveIcon className='button-icon' />
        </button>
        <div id='image'>
          <button
            type='button'
          ...
          >
            <InsertImageIcon className='button-icon' />
          </button>
            ...
            <div className='image-selector-show'>
              <div className='image-option-button'>
                ...
                <div className='image-insert-url'>
                  ...
                  <button
                    type='button'
                    className='image-insert-url-button'
                    onClick={() => {
          ...
          <div className='symbol-selector-container'>
            <div className='symbol-selector-show'>
              <div className='symbol-option-button'>
                <button
                  type='button'
                  onClick={() => onCommand('insertHTML', '&copy;')}
                >
                  ©
                </button>
                <button
                  type='button'
                  onClick={() => onCommand('insertHTML', '&euro;')}
                >
                  €
                </button>
                <button
                  type='button'
                  onClick={() => onCommand('insertHTML', '&trade;')}
                >
                  ™
                </button>
                <button
      ...
      >
        ...
        >
          <RedoIcon className='button-icon' />
        </button>
      </div>
    </div>
  );
